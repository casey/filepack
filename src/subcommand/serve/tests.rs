...
#[test]
fn media_video_item_cover_response() {
  let server = TestServer::new();

  let mut video = Item::<Video>::test("foo.mp4");
  video.content.cover = Some(Image::test("bar.png"));
  ...
  server
    .get(format!("/media/video/{fingerprint}/item/1/cover"))
    .assert_header(header::ACCEPT_RANGES, "bytes")
    .assert_header(header::CACHE_CONTROL, "public, max-age=31536000, immutable")
    .assert_header(header::CONTENT_LENGTH, "3")
    .assert_header(header::CONTENT_TYPE, "image/png")
  ...
  server
    .get(format!("/media/video/{fingerprint}/item/2/cover"))
    .status(StatusCode::NOT_FOUND)
    .assert_body(format!(
      "video 2 in package {fingerprint} does not have a cover"
    ))
    .send();

  server
    .get(format!("/media/video/{fingerprint}/item/3/cover"))
    .status(StatusCode::NOT_FOUND)
    .assert_body(format!(
