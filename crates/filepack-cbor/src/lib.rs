...
fn parse_fields(input: &Input) -> syn::Result<Vec<ParsedField<'_>>> {
...
pub fn derive_encode(input: TokenStream) -> TokenStream {
  ...
  match derive_encode_inner(&input) {
    Ok(tokens) => tokens.into(),
    Err(err) => err.to_compile_error().into(),
  }
}

fn derive_encode_inner(input: &Input) -> syn::Result<proc_macro2::TokenStream> {
...
pub fn derive_decode(input: TokenStream) -> TokenStream {
  ...
  match derive_decode_inner(&input) {
    Ok(tokens) => tokens.into(),
    Err(err) => err.to_compile_error().into(),
  }
}

fn derive_decode_inner(input: &Input) -> syn::Result<proc_macro2::TokenStream> {
  let name = &input.ident;
  let fields = parse_fields(input)?;
